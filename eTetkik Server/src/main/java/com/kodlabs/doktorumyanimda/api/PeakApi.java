...
public class PeakApi {
    ...
    public ResponseEntitySet<String> create(@Context HttpServletRequest hsr, CreatePeakRequest request, @QueryParam("version")String version, @QueryParam("os")String os){
        return Managers.peakManager.create(request, version, os, Functions.getClientIpAddress(hsr));
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public ResponseEntitySet<Boolean> isFirstVisitFree(@QueryParam("patientID")String patientID, @QueryParam("doctorID")String doctorID){
    ...
    @Path("/detail")
    @Produces(MediaType.APPLICATION_JSON)
    public ResponseEntitySet<Peak> list(@QueryParam("peakID")String peakID, @QueryParam("userID")String userID, @QueryParam("role") byte role){
    ...
    public ResponseEntity peakVerifyDemand(@Context HttpServletRequest hsr, PeakDemandVerifyRequest request){
        return Managers.peakManager.peakDemandVerify(request, Functions.getClientIpAddress(hsr));
    }
    @GET
    @Path("/fee/list")
    @Produces(MediaType.APPLICATION_JSON)
