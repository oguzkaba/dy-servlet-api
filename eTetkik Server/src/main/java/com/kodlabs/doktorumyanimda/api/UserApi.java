...
public class UserApi {
    ...
    @Path("/contact")
    @GET
    public ResponseEntitySet<Contact> contactInformation(@QueryParam("uname") String userID,
    ...
    @GET
    public ResponseEntitySet<Contact> userAvailableContact(@PathParam("uname") String uname,
            @PathParam("role") byte role) {
    ...
    @GET
    public ResponseEntitySet<Boolean> userPasswordVerify(@PathParam("userID") String userID,
            @PathParam("role") byte role, @QueryParam("password") String password) {
        return Managers.userManager.userPasswordVerify(userID, role, password);
    }

    @Path("/signup/phone/exists")
    @GET
    public ResponseEntity singUpPhoneExists(@QueryParam("phone") String phone, @QueryParam("role") byte role,
            @QueryParam("verifyToken") String verifyToken) {
        try {
    ...
    @PUT
    public ResponseEntity changePassword(@PathParam("userID") String userID, @PathParam("role") byte role,
            ChangePassword data) {
    ...
    @Path("/all")
    @GET
    public ResponseEntitySet<List<UserInformation>> allUser(@QueryParam("userID") String userID,
    ...
    @PUT
    public ResponseEntity resetPassword(@PathParam("userID") String userID, @PathParam("role") byte role,
            ResetPassword data) {
    ...
    @GET
    public ResponseEntity contactAddressControl(@PathParam("role") byte role, @QueryParam("address") String address,
            @QueryParam("type") String type) {
        IIntegrations integration;
        String code = Functions.generateCode();
        MailData mailData = MailData.builder()
                .title(Messages.mailVerification)
                .address(address)
                .message(code)
                .type(MailTypes.SUPPORT.getType())
                .userName("").build();
        switch (ContactType.findContactType(type)) {
            case SMS:
                ...
                if (response.isSuccess) {
                    integration = IntegrationsFactory.getIntegrations(
                            new SmsData(String.format(Messages.smsLoginVerifyMessage, code), address),
                            IntegrationsFactory.SMS);
                    break;
                } else {
                    return response;
            ...
            case MAIL:
                ...
                integration = IntegrationsFactory.getIntegrations(mailData, IntegrationsFactory.MAIL);
                break;
            default:
    ...
    public ResponseEntity sendVerifyCode(SendVerifyCodeRequest request) {
        if (request.isValid()) {
            ResponseEntitySet<UserContactInformation> responseContact = Managers.userManager
                    .userContactInformation(request.getUname(), request.getRole());
            String userName = Managers.userManager.getFullName(request.getUname(), request.getRole());

            if (responseContact.isSuccess) {
                String code = Functions.generateCode();
                IIntegrations integrations;
                MailData mailData = MailData.builder()
                        .title(Messages.passwordReset)
                        .address(responseContact.getData().getMail())
                        .message(String.format(Messages.passwordForgotCodeVerifyMessage, code))
                        .type(MailTypes.SUPPORT.getType())
                        .userName(userName).build();
                switch (ContactType.findContactType(request.getContactType())) {
                    case SMS:
                        integrations = IntegrationsFactory.getIntegrations(
                                new SmsData(String.format(Messages.passwordForgotCodeVerifyMessage, code),
                                        responseContact.getData().getPhone()),
                                IntegrationsFactory.SMS);
                        break;
                    case MAIL:
                        integrations = IntegrationsFactory.getIntegrations(
    ...
    @GET
    public ResponseEntitySet<String> forgotCodeVerify(@PathParam("uname") String uname, @PathParam("role") byte role,
