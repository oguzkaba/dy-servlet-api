...
public class PatientManager {
    ...
    public ResponseEntitySet<String> loginV2(PatientLoginV2Request request, String ip){
        if(!request.isValid()){
            return new ResponseEntitySet<>(false, ErrorMessages.inValidData);
        }
        switch (LoginType.getInstance(request.getLoginType())){
            case PHONE:
                if(!Patterns.PHONE.matcher(request.getUname()).matches()){
                    patientLogSend("loginV2", request.getUname(), ip, LogEventDescription.LOGIN_INVALID_PHONE_UNAME.getMessage(), Role.PATIENT);
                    return new ResponseEntitySet<>(false, ErrorMessages.inValidPhone);
                }
                break;
            case TC_NUMBER:
                if(!Patterns.TC_NO.matcher(request.getUname()).matches()){
                    patientLogSend("loginV2", request.getUname(), ip, LogEventDescription.LOGIN_INVALID_TC_UNAME.getMessage(), Role.PATIENT);
        ...
        try{
            ...
            if(response.isSuccess){
                ...
                if(!Common.isLocal  && !Phones.isContains(request.getUname(), false) ){
                    IIntegrations integration = IntegrationsFactory.getIntegrations(new SmsData(String.format(Messages.smsLoginVerifyMessage,
                ...
                patientLogSend("loginV2", request.getUname(), ip, LogEventDescription.LOGIN_FAILED.getMessage(), Role.PATIENT);
    ...
    public ResponseEntitySet<UserPatient> singUpV2(PatientSingUpV2Request request, String ip){
        if(!request.isValid()){
            return new ResponseEntitySet<>(false, ErrorMessages.inValidData);
        }
        try{
            if(NVIService.getInstance().verifyNvi(request)){
                ...
                if(response.isSuccess){
                    patientLogSend("singUpV2", response.getData().getUserID(), ip, LogEventDescription.SING_UP.getMessage(), Role.PATIENT);
                }
                return response;
            }else{
                        ...
                        if(integration != null){
                            ...
                            if(result){
                                ...
                                if(!res.isSuccess){
                                    ...
                                    System.out.println(format.format(Calendar.getInstance().getTime()).concat("Patient Login New Code increase error: ".concat(res.message)));
                                }
                            }else{
    ...
    public ResponseEntitySet<UserPatient> loginVerify(PatientLoginVerifyRequest request, String ip){
        if(!request.isValid()){
            return new ResponseEntitySet<>(false, ErrorMessages.inValidData);
        }
        try{
            ...
            if(response.isSuccess && !Common.isLocal){
                new Thread(()->{
                    Map<String, Object> info = new HashMap<>();
                    IEvents events = EventsFactory.getEvents(new AzureEventsEntity(EventsType.APPROVAL_CODE_EVENTS, "ApprovalCode",  info));
                    if(events != null){
                        events.insert();
                    }
                }).start();
            ...
            if(response.isSuccess){
                patientLogSend("loginVerify", response.getData().getUserID(), ip, LogEventDescription.LOGIN.getMessage(), Role.PATIENT);
            }else{
                patientLogSend("loginVerify", request.getPhone(), ip, LogEventDescription.LOGIN_VERIFY_FAILED.getMessage(), Role.PATIENT);
            }
            return response;
        }catch (ConnectionException e){
            return new ResponseEntitySet<>(false, e.getLocalizedMessage());
    ...
    private void patientLogSend(String methodName,final String phoneOrUserID, String ip, String eventDescription, Role role){
        new Thread(()->{
            try {
    ...
    public ResponseEntity inspectionContentCreate(String ip, InspectionContent inspectionContent){
        if(inspectionContent == null || !inspectionContent.isValid() || TextUtils.isEmpty(inspectionContent.getInspectionID())){
            return new ResponseEntity(false, ErrorMessages.inValidData);
        }
        try{
            if(this.patientDal.inspectionExists(inspectionContent.getInspectionID())){
                ...
                if(response.isSuccess){
                    ...
                    patientLogSend("inspectionContentCreate", id, ip, LogEventDescription.INSPECTION_CONTENT_CREATE.getMessage(), Role.DOCTOR);
                }
                return response;
            }else{
    ...
    public ResponseEntity inspectionContentUpdate(String ip,InspectionContent inspectionContent){
        ...
        try{
           if(this.patientDal.inspectionContentExists(inspectionContent.getId())){
               ...
               if(response.isSuccess){
                   patientLogSend("inspectionContentUpdate", inspectionContent.getId(), ip, LogEventDescription.INSPECTION_CONTENT_UPDATE.getMessage(), Role.DOCTOR);
               }
               return response;
           }else{
    ...
    public ResponseEntity inspectionContentDelete(String ip, String inspectionContentID){
        ...
        try{
            ...
            if(response.isSuccess){
                patientLogSend("inspectionContentDelete", inspectionContentID, ip, LogEventDescription.INSPECTION_CONTENT_DELETE.getMessage(), Role.DOCTOR);
            }
            return response;
        }catch (ConnectionException e){
            return new ResponseEntity(false, e.getLocalizedMessage());
    ...
    public ResponseEntity sysTakipNoCreate(String ip, PatientSysTakipNoCreateRequest request){
        if(!request.isValid()){
            return new ResponseEntity(false, ErrorMessages.inValidData);
        }
        try{
            ...
            if(response.isSuccess){
                patientLogSend("sysTakipNoCreate", request.getDoctorID(), ip, String.format(LogEventDescription.SYS_TAKIP_NO_CREATE.getMessage(), request.getTcNumber()), Role.DOCTOR);
            }
            return response;
        }catch (ConnectionException e){
            return new ResponseEntity(false, e.getLocalizedMessage());
    ...
    public ResponseEntity sysTakipNoUpdate(String ip, PatientSysTakipNoUpdateRequest request){
        if(!request.isValid()){
            return new ResponseEntity(false, ErrorMessages.inValidData);
        }
        try{
            ...
            if(response.isSuccess){
                if(response.isSuccess){
                    patientLogSend("sysTakipNoUpdate", request.getDoctorID(), ip, String.format(LogEventDescription.SYS_TAKIP_NO_CREATE.getMessage(), request.getTcNumber()), Role.DOCTOR);
                }
            }
            return response;
        }catch (ConnectionException e){
