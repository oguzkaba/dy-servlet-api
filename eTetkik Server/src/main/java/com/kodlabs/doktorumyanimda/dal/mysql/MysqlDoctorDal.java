...
public class MysqlDoctorDal implements IDoctorDal {
    @Override
    public ResponseEntitySet<List<DoctorInformationForAdmin>> list(String userID, byte role)
    ...
    @Override
    public ResponseEntitySet<List<DoctorInformation>> notAnonymousList(String branch, String city, String os)
            throws ConnectionException {
        if (MysqlConnection.getInstance() == null) {
            throw new ConnectionException();
        }
    ...
    public ResponseEntitySet<Contact> contact(String userID) throws ConnectionException {
        ...
        try {
            ...
            if (resultSet != null && resultSet.next()) {
                response = new ResponseEntitySet<>(
                        new Contact(resultSet.getString("email"), resultSet.getString("phone")));
            } else {
    ...
    public ResponseEntity register(DoctorRegister request) throws ConnectionException {
        ...
        ResponseEntity response;
        try {
            statement = MysqlConnection.getInstance().prepareCall(
                    "{ CALL doctorRegister(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,  ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) }");
            ...
            statement.setString(4, Functions.toSHA256(request.getPassword()));
    ...
    public ResponseEntity changePassword(ChangePasswordRequest request) throws ConnectionException {
        ...
        try {
            ...
            statement.setString(1, Functions.toSHA256(request.getPassword()));
            statement.setString(2, Functions.toSHA256(request.getNewPassword()));
    ...
    @Override
    public ResponseEntitySet<LoginData> loginV2(DoctorLoginV2Request request, String verifyCode)
            throws ConnectionException {
        if (MysqlConnection.getInstance() == null) {
            throw new ConnectionException();
        }
    ...
    @Override
    public ResponseEntity updateSideAdmin(String userID, byte role, DoctorBaseProfile profile)
            throws ConnectionException {
        if (MysqlConnection.getInstance() == null) {
            throw new ConnectionException();
        }
        ...
        try {
            statement = MysqlConnection.getInstance().prepareStatement(
                    "{ CALL  doctorDetailUpdateSideAdmin(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ) }");
            statement.setString(1, userID);
            statement.setByte(2, role);
    ...
    public Optional<DoctorRecipeInformation> recipeInformation(String userID) throws ConnectionException {
        ...
        try {
            statement = MysqlConnection.getInstance().prepareStatement(
                    "SELECT facilityID, medula_password, branch_code, certificate_code FROM doctor LEFT JOIN doctor_recipe_information ON doctor.userID = doctor_recipe_information.doctorID WHERE doctorID = ?");
            statement.setString(1, userID);
            resultSet = statement.executeQuery();
            if (resultSet.next()) {
    ...
    @Override
    public void recipeInformationCreate(DoctorRecipeInformationCreate doctorRecipeInformationCreate)
            throws ConnectionException, SQLException {
        if (MysqlConnection.getInstance() == null) {
            throw new ConnectionException();
        }
        DoctorRecipeInformation information = doctorRecipeInformationCreate.getRecipeInformation();
        PreparedStatement statement = MysqlConnection.getInstance().prepareStatement(
                "INSERT INTO doctor_recipe_information(medula_password, branch_code, certificate_code, doctorID, create_date) VALUE (?, ?, ?, ?, NOW())");
    ...
    @Override
    public void recipeInformationUpdate(DoctorRecipeInformationCreate doctorRecipeInformationCreate)
            throws ConnectionException, SQLException {
        if (MysqlConnection.getInstance() == null) {
            throw new ConnectionException();
        }
        DoctorRecipeInformation information = doctorRecipeInformationCreate.getRecipeInformation();
        PreparedStatement statement = MysqlConnection.getInstance().prepareStatement(
                "UPDATE doctor_recipe_information SET medula_password = ?, branch_code = ?, certificate_code = ? WHERE doctorID = ?");
        statement.setString(1, information.getMedulaPassword());
        statement.setInt(2, information.getBranchCode());
        statement.setInt(3, information.getCertificateCode());
        statement.setString(4, doctorRecipeInformationCreate.getDoctorID());
    ...
    public boolean isExistsRecipeInformation(String userID) throws ConnectionException, SQLException {
        ...
        PreparedStatement statement = MysqlConnection.getInstance().prepareStatement(
                "SELECT IF(EXISTS(SELECT * FROM doctor_recipe_information WHERE doctorID = ?), 1, 0) AS result");
        statement.setString(1, userID);
